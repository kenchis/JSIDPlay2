...
public enum AudioOp {
	...
	private static final Map<Integer, AudioOp> lookup;
	static {
		Map<Integer, AudioOp> result = new HashMap<>();
		for (AudioOp audioOp: AudioOp.values()) {
			result.put(audioOp.getAudioOp(), audioOp);
		}
		lookup = result;
	}
