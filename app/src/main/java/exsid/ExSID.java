...
//	#endif	// EXSID_THREADED
	 ...
	 */
	private void xSwrite(byte[] buff, int size) throws D2xxManager.D2xxException {
		device.write(buff, size);
	}
	 ...
	 */
	private int xSread(byte[] buff, int size) throws D2xxManager.D2xxException, InterruptedException {
//	#ifdef	EXSID_THREADED
...
//	#ifdef	EXSID_THREADED
	...
	private Thread exSIDthreadOutput = new Thread(new Runnable() {
		...
		public void run() {
			...
			} catch (D2xxManager.D2xxException | InterruptedException e) {
				e.printStackTrace();
			}
		}
	});
//	#endif	// EXSID_THREADED
	 ...
	 * @throws InterruptedException
	 */
	private void xSoutb(byte b, int flush) throws D2xxManager.D2xxException, InterruptedException {
...
//	#endif
	 ...
	 */
	@Override
	public int exSID_init(Context mContext) {
		...
		try {
			...
			for (SupportedDevices xSsup : xSsupported) {
				if (deviceList[0].description.equals(xSsup.getDescription())) {
					hardwareSpecs = xSsup.getHardwareSpecs();
				}
			}
			if (hardwareSpecs == null) {
				return -1;
			}
...
//		#endif
		...
		} catch (D2xxManager.D2xxException | InterruptedException e1) {
			e1.printStackTrace();
			return -1;
		}
	}
...
//		#endif
	 ...
	 * @throws InterruptedException
	 */
	private void xSdelay(long cycles) throws D2xxManager.D2xxException, InterruptedException {
	...
	private void xSlongdelay(long cycles) throws D2xxManager.D2xxException, InterruptedException {
		int multiple;
		int flush;
		long delta;
...
//				}
	 ...
	 * @throws InterruptedException
	 */
	private void exSID_write(byte addr, byte data, int flush) throws D2xxManager.D2xxException, InterruptedException {
		xSoutb(addr, 0);
		xSoutb(data, flush);
	}
	 ...
	 * @throws InterruptedException
	 */
	private byte exSID_read(byte addr, int flush) throws D2xxManager.D2xxException, InterruptedException {
	 ...
	 * @throws D2xxManager.D2xxException
	 */
	private void xSfw_usb_setup(int baudrate, byte latency) throws D2xxManager.D2xxException, IllegalArgumentException {
		device.setBaudRate(baudrate);
		device.setDataCharacteristics(D2xxManager.FT_DATA_BITS_8, D2xxManager.FT_STOP_BITS_1, D2xxManager.FT_PARITY_NONE);
		device.setFlowControl(D2xxManager.FT_FLOW_NONE, (byte) 0, (byte) 0);
		device.setLatencyTimer(latency);
	}

	private void xSfw_usb_purge_buffers() throws D2xxManager.D2xxException {
		device.purge((byte) (D2xxManager.FT_PURGE_RX  | D2xxManager.FT_PURGE_TX));
	}

	private void xSfw_usb_close() throws D2xxManager.D2xxException {
		device.close();
	}
